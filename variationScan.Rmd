...
Chronic Obstructive Pulmonary Disease (COPD) is a lung condition characterized by breathing difficulties. Previously it was studied the gene expression profiles of these diseases and published in PulmonDB database. But it was unknown what variants cause the disease, which is the main objective of my project and to determine which variants change the expression of the previously analyzed genes.        

To do that, I focused on retrieving the variants, analyzed them with two negative controls if they had a significant change by motifs. Then I searched for genes that were close to the variants and saw if some of them were in a meta-analysis (realized by Ana Beatriz Villaseñor) which complement the data from PulmonDB.        
</div>      

![Figure 1:Flowchart of the tools used. ](/home/aschafer/Documentos/Genomicas/Laboratorios/RegGenoLab/Variations/COPD/Flowchart.png)
...
<div style = "text-align: justify">
I started filtering the variants related to COPD, FEV1, and FVC. I created a bed file in which I kept the chromosome name, the start position, and the end. I also needed a vcf file; I base the columns that I needed from the vcf manual.        
</div>
...
<div style = "text-align: justify">
Here I concatenate all the variants retrieve from the disease and gene association variants to end up with the final vcf and bed files. Two variants created a little bit of a problem because the reference allele was in the IUPAC code, so I eliminated them from both files. In this step, I also put on the corresponding header in each file.         
</div> 
...
<div style = "text-align: justify">          
Once I had the results from variation-Scan, I analyze them. To do that, I filtered just the ones that fulfill quality requirements, and using the JASPAR data I added a column with the corresponding TF from the motif.  
</div> 
...
<div style = "text-align: justify"> 
To make sure that my variants were affecting the TF, I realized two different negative controls. One with vSampler and the other one with matrix permutations, both explain later. And I compute both of them for tissue and blood data independently.      
</div>      
...
### vSampler
<div style = "text-align: justify">
vSampler is a tool that receives the list of variants that you're using, and for each one of them retrieves random variants given eight different properties. This is used to compute a negative control.    
To use vSampler, I needed to create a file with the chromosome name and the variant's position. For that, I used only the ones that were significant from variationScan and with that created the input file.         
</div>
...
<div style = "text-align: justify">
Again I used some tools from RSAT to perform the negative control, to do that I created 5 permutations of my matrices and then ran again variation-scan with the new matrix file. 
</div>  
...
### Number of variant and transcriptional factors for each gene     

```{r}
# gene, num_variants, num_tfs, gene TE
variant_tf_counter <- function(x, data, s){
  gene_data <- data %>% filter(gene == x)
  return(c(x, format(length(unique(gene_data$var_id)), nsmall = 0), length(unique(gene_data$TF)), s))
}
variants_tfs_count <- rbind(as.data.frame(t(sapply(X = unique(Tissue_data$gene), function(x){return(variant_tf_counter(x, Tissue_data, "tissue"))}))), as.data.frame(t(sapply(X = unique(Blood_data$gene), function(x){return(variant_tf_counter(x, Blood_data, "blood"))}))))
rownames(variants_tfs_count) <- NULL
colnames(variants_tfs_count) <- c("gene", "num_variants", "num_TFs", "analysis")
variants_tfs_count <-variants_tfs_count[order(as.numeric(variants_tfs_count$num_TFs), decreasing = T),]
write.table(variants_tfs_count, file = "Results/Tables/Gene_variants_tfs_counts.csv", row.names = F, quote = F, sep = "\t")
head(variants_tfs_count) %>% htmlTable
```

### Variants and TFs for each gene 
...
### variant, gene and TFs association     
...
```     
![Figure 2: TP53 consequences](/home/aschafer/Documentos/Genomicas/Laboratorios/RegGenoLab/Variations/COPD/Results/Plots/TP53_consequences.png)
